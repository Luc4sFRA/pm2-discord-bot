...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('pm2-start')
        .setDescription('Démarrer un processus PM2 avec un ID spécifié.')
        .addStringOption((option) =>
            option
                .setName('process_id')
                .setDescription('ID du processus PM2 à démarrer')
                .setRequired(true)
        ),
    async execute(interaction) {
        ...
        console.log(`Verrouillage du processus ${processId}: ${lockedProcesses.includes(processId)}`);
        ...
        pm2.connect((err) => {
            ...
            pm2.list((err, processes) => {
                ...
                pm2.start(startOptions, async (err, apps) => {
                    if (err) {
                        console.error(`Erreur lors du démarrage de PM2 : ${err.message}`);
                        const embed = new EmbedBuilder()
                            .setDescription(`\`❌\` Erreur lors du démarrage du processus : ${err.message}`)
                            .setColor(config.color);
                        return interaction.reply({ embeds: [embed] });
                    }

                    const embed = new EmbedBuilder()
                        .setDescription(`\`✅\` Processus \`${processId}\` (${process.name}) démarré avec succès.`)
                        .setColor(config.color);
                    ...
                    if (logChannelId) {
                        ...
                        if (logChannel && logChannel.isTextBased()) {
                            const logEmbed = new EmbedBuilder()
                                .setDescription(`\`✅\` Le processus \`${processId}\` (${process.name}) a été démarré par ${interaction.user.tag}.`)
                                .setColor(config.color);
